...
//
    if (GetIsPC(oPC))
        ...
        //Persistant
        string sFaction = GetCampaignString("SOTS", "Faction", oPC);
...
//            AddPersistentJournalQuestEntry("1001", 1, oPC, FALSE, FALSE, FALSE);
            ...
            SetCampaignInt("SOTS","iMana", 0, oPC);
...
//
    if (GetCampaignInt("SOTS", "PureCaster", oPC)==1)
    {
        effect eEffect;
...
void ZEP_PurifyItem(object oItem, object oPC, int nIsEntering)
{
  ...
  if (GetLocalInt(oItem, "ZEP_CR_TEMPITEM"))
      ...
      SendMessageToAllDMs(sDupeReport);
						 ...
						 // Custom OnClientEnter Script for DMFI 101
...
// Checks if entering is a DM, provides DMFI wands exploder if needed, then exits
...
if (GetIsDM(oPC))
    ...
    SetLocalInt(oPC, "DM_tag", 1); // sets DM tag on PC
    return;
    }

// CreateItemOnObject("dmfi_pc_emote",oPC,1); // Gives PC emote wand
