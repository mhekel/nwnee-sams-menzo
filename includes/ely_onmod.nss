...
// database filename
const string PJQ_DATABASE       = "JOURNALS";
// database fieldname
const string PJQ_PLAYER_VARNAME = "QUESTJOURNAL";
...
void RebuildJournalQuestEntries(object oCreature)
{
  ...
  {
    string sEntries = GetCampaignString(PJQ_DATABASE,PJQ_PLAYER_VARNAME,oCreature);
...
void StoreQuestEntry(string szPlotID, int nState, object oCreature, int bAllowOverrideHigher=FALSE)
{
  ...
  // store quest entries
  SetCampaignString(PJQ_DATABASE,PJQ_PLAYER_VARNAME,sEntries,oCreature);
}
